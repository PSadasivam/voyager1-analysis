#!/usr/bin/env python3
"""
voyager1_astropy_science.py
---------------------------------

This script demonstrates how to fetch and visualise the heliocentric position of
Voyager 1 relative to Earth using the JPL HORIZONS service via
``astroquery``.  It also contains helpers for plotting time–series data from
    """En    parser = argparse.ArgumentParser(description='Visualise Voyager 1 ephemeris and science data')
    ...
    args = parser.parse_args(argv)point for the command line interface."""
    ...
    parser.add_argument('--days-back', type=int, default=7,
                       help='Number of days back to fetch real data (default: 7, only used with --real-data)')
    args = parser.parse_args(argv)‑separated values (CSV) files or Common Data Format (CDF) files.  The
intent is to provide a simple, extensible entry point for analysing Voyager 1
science data.

If ``astroquery`` or an internet connection is unavailable, the script falls
back to approximate sample positions.  A small sample CSV file is included
(see ``voyager1_magnetometer_sample.csv``) to illustrate how instrument data
can be plotted.
"""
...
def fetch_ephemeris(now: datetime.datetime) -> tuple[np.ndarray, np.ndarray]:
    ...
    try:
        ...
        obj_voyager = Horizons(id="Voyager 1", location="@sun", epochs=epoch, id_type='majorbody')
...
def plot_positions(v_pos: np.ndarray, e_pos: np.ndarray) -> None:
    ...
    ax.scatter(0, 0, 0, label='Sun', s=50)
    ax.scatter(e_pos[0], e_pos[1], e_pos[2], label='Earth', s=50)
    ax.scatter(v_pos[0], v_pos[1], v_pos[2], label='Voyager 1', s=50)
    ax.set_xlabel('X (AU)')
    ax.set_ylabel('Y (AU)')
    ax.set_zlabel('Z (AU)')
...
def fetch_real_voyager_data_alternative() -> Path:
    """Alternative method to fetch Voyager 1 data using a direct CSV approach.
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.csv:
        ...
        sample_csv = Path(__file__).resolve().parent / 'voyager1_magnetometer_sample.csv'
